...
function clge_migrate_add_alias_description()
{
    ...
    if (empty($alias_exists)) {
        dbDelta(
            "ALTER TABLE $table_name ADD COLUMN alias varchar(50) DEFAULT NULL AFTER abrev",
        );
    }

    // Verifier si la colonne description existe
    ...
    if (empty($description_exists)) {
        dbDelta(
            "ALTER TABLE $table_name ADD COLUMN description text DEFAULT NULL AFTER alias",
        );
    }
}
...
function clge_create_event($data)
{
    ...
    $insert_data = [
        "debut" =>
            $data["debut"] instanceof DateTime
                ? $data["debut"]->format("Y-m-d H:i:s")
                : $data["debut"] .
                    (array_key_exists("debut_h", $data)
                        ? " " . $data["debut_h"]
                        : ""),
        "fin" =>
            $data["fin"] instanceof DateTime
                ? $data["fin"]->format("Y-m-d H:i:s")
                : $data["fin"] .
                    (array_key_exists("fin_h", $data)
                        ? " " .
                            ($data["fin_h"] != "00:00"
                                ? $data["fin_h"]
                                : "23:59")
    ...
    if (isset($data["description"])) {
        $insert_data["description"] = sanitize_textarea_field(
            $data["description"],
        );
    }
...
function clge_update_event($id, $data)
{
    ...
    if (isset($data["lieu_physique"])) {
        $update_data["lieu_physique"] = sanitize_text_field(
            $data["lieu_physique"],
        );
    }
    if (isset($data["url"])) {
    ...
    if (isset($data["description"])) {
        $update_data["description"] = sanitize_textarea_field(
            $data["description"],
        );
    }
    ...
    $wpdb->update($table_name, $update_data, ["id" => $id], null, ["%d"]);
